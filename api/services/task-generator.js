...
export async function generateTasksFromPRD({ prdContent, numTasks, useResearch, apiKeys }) {
  ...
  const tempPrdPath = path.join(tempDir, `prd-${crypto.randomBytes(8).toString('hex')}.txt`);
  const tempOutputPath = path.join(tempDir, `tasks-${crypto.randomBytes(8).toString('hex')}.json`);
