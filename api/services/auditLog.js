...
export const AUDIT_EVENTS = {
	...
	AUTH_EMAIL_VERIFICATION_RESENT: 'auth.email_verification.resent',
...
export class AuditLogService {
	...
	async logEvent(eventData) {
		try {
			...
			if (
				eventData.riskLevel === RISK_LEVELS.HIGH ||
				eventData.riskLevel === RISK_LEVELS.CRITICAL
			) {
	...
	logToConsole(auditLog) {
		...
		if (
			auditLog.risk_level === RISK_LEVELS.CRITICAL ||
			auditLog.risk_level === RISK_LEVELS.HIGH
		) {
	...
	async queryLogs(filters = {}) {
		let query = supabase.from('audit_logs_enhanced').select('*');
...
export const logAuthEvent = (eventType, data) => {
	return auditLogger.logEvent({
		...
		riskLevel: eventType.includes('failed')
			? RISK_LEVELS.MEDIUM
			: RISK_LEVELS.LOW,
		...data
	});
