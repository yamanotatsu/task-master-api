...
class SecurityService {
   ...
   */
  async trackLoginAttempt(identifier, identifierType = 'email', success = false, metadata = {}) {
    try {
  ...
  async isAccountLocked(identifier, identifierType = 'email') {
    try {
      ...
      if (!countError && count >= this.maxLoginAttempts) {
        // Create lock
        await this.lockAccount(identifier, identifierType, 'Exceeded maximum login attempts');
   ...
   * Lock an account
   */
  async lockAccount(identifier, identifierType = 'email', reason = 'Security violation') {
    try {
   ...
   * Block an IP address
   */
  async blockIP(ipAddress, reason = 'Suspicious activity', duration = 24 * 60 * 60 * 1000) {
    try {
  ...
  async calculateDelay(identifier, identifierType = 'email') {
    try {
      ...
      const exponentialDelay = Math.min(
        Math.pow(this.progressiveDelayFactor, attempts - 10) * 10000,
    ...
    try {
      ...
      if (recentAttempts && recentAttempts.length > 0) {
        ...
        if (uniqueIPs > 3 && recentAttempts.length > 10) {
          await this.flagSuspiciousActivity(identifier, 'Multiple IPs attempting access');
        ...
        if (recentAttempts.length > 1) {
          ...
          for (let i = 1; i < recentAttempts.length; i++) {
            const diff = new Date(recentAttempts[i-1].attempted_at) - new Date(recentAttempts[i].attempted_at);
            timeDiffs.push(diff);
          }
          
          const avgTimeDiff = timeDiffs.reduce((a, b) => a + b, 0) / timeDiffs.length;
          if (avgTimeDiff < 1000) { // Less than 1 second average
            await this.flagSuspiciousActivity(identifier, 'Automated attack detected');
