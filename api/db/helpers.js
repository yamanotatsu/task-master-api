...
export async function getProjectStatistics(projectId) {
  ...
  tasks?.forEach(task => {
    ...
    if (task.status === 'completed' || task.status === 'done') {
      ...
      const completionDays = Math.ceil((updated - created) / (1000 * 60 * 60 * 24));
      totalCompletionTime += completionDays;
      completedCount++;
    }
    
    // Check upcoming deadlines
    if (task.deadline && task.status !== 'completed' && task.status !== 'done') {
  ...
  return {
    ...
    upcomingDeadlines: upcomingDeadlines.sort((a, b) => 
