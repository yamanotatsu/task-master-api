	...
	try {
		...
		if (!result.success) {
			...
			if (
				...
				result.error?.message?.includes('cycle') ||
				result.error?.code === 'CIRCULAR_DEPENDENCY'
			) {
				errorCode = 'CIRCULAR_DEPENDENCY';
			} else if (
				result.error?.message?.includes('already exists') ||
				result.error?.code === 'DEPENDENCY_EXISTS'
			) {
	...
	try {
		...
		if (!result.success) {
			...
			return res.status(statusCode).json({
				...
				error: {
					code:
						statusCode === 404 ? 'TASK_NOT_FOUND' : 'REMOVE_DEPENDENCY_ERROR',
					message: result.error
				}
			});
		}
...
export async function validateDependenciesHandler(req, res) {
	try {
		...
		const result = await validateDependenciesDirect(args, logger, {
			session: {}
		});
		...
		if (validation.data.autoFix && result.issues && result.issues.length > 0) {
			...
			const fixResult = await fixDependenciesDirect(fixArgs, logger, {
				session: {}
			});
