	...
	try {
      ...
      `
		...
		const descriptionLength =
			(task.description?.length || 0) + (task.details?.length || 0);
		...
		if (subtaskCount === 0 && complexityScore > 3)
			riskFactors.push('未分解の複雑なタスク');
		if (task.priority === 'high' && dependencyCount > 0)
			riskFactors.push('高優先度かつ依存関係あり');
	...
	try {
      ...
      `);
		...
		const complexityDistribution = {
			...
			medium: report.filter(
				(t) => t.complexityScore > 2 && t.complexityScore <= 3.5
			).length,
			high: report.filter(
				(t) => t.complexityScore > 3.5 && t.complexityScore <= 4.5
			).length,
