...
import { logOrganizationEvent, logSecurityEvent, AUDIT_EVENTS, RISK_LEVELS } from '../services/auditLog.js';
    ...
    if (name.trim().length > 100) {
      return res.status(400).json({
        ...
        error: {
          ...
          details: [{ field: 'name', message: 'Name must be 100 characters or less' }]
        }
      });
    }
...
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    projectCounts?.forEach(project => {
      projectCountMap[project.organization_id] = (projectCountMap[project.organization_id] || 0) + 1;
    });
 ...
 */
router.get('/:organizationId', authMiddleware, requireRole('member'), async (req, res) => {
  try {
    ...
    res.status(500).json({
      ...
      error: {
        code: 'INTERNAL_ERROR',
        message: 'An unexpected error occurred while retrieving organization details'
      }
    });
  }
});
 ...
 */
router.put('/:organizationId', authMiddleware, requireRole('admin'), async (req, res) => {
  try {
    const { organizationId } = req.params;
    const { name, description } = req.body;
 ...
 */
router.delete('/:organizationId', authMiddleware, requireRole('admin'), async (req, res) => {
  try {
    const { organizationId } = req.params;
    ...
    if (admins.length > 1) {
      return res.status(409).json({
        ...
        error: {
          ...
          message: 'Cannot delete organization with other admins. Please remove other admins first.'
        }
      });
    }
 ...
 */
router.post('/:organizationId/invites', authMiddleware, requireRole('admin'), async (req, res) => {
  try {
    const { organizationId } = req.params;
    ...
    if (!['admin', 'member'].includes(role)) {
      return res.status(400).json({
        ...
        error: {
          ...
          details: [{ field: 'role', message: 'Role must be either "admin" or "member"' }]
        }
      });
    }
    ...
    try {
      ...
      await sendInvitationEmail({
        to: email.trim(),
        organizationName: req.organizationMember.organization?.name || 'the organization',
 ...
 */
router.get('/:organizationId/invites', authMiddleware, requireRole('admin'), async (req, res) => {
  try {
    const { organizationId } = req.params;
    const page = parseInt(req.query.page) || 1;
    const limit = Math.min(parseInt(req.query.limit) || 20, 100);
 ...
 */
router.delete('/:organizationId/invites/:inviteId', authMiddleware, requireRole('admin'), async (req, res) => {
  try {
 ...
 */
router.post('/:organizationId/invites/:token/accept', optionalAuth, async (req, res) => {
  try {
 ...
 */
router.get('/:organizationId/members', authMiddleware, requireRole('member'), async (req, res) => {
  try {
    const { organizationId } = req.params;
    const page = parseInt(req.query.page) || 1;
    const limit = Math.min(parseInt(req.query.limit) || 20, 100);
 ...
 */
router.put('/:organizationId/members/:profileId', authMiddleware, requireRole('admin'), async (req, res) => {
  try {
 ...
 */
router.delete('/:organizationId/members/:profileId', authMiddleware, requireRole('admin'), async (req, res) => {
  try {
    const { organizationId, profileId } = req.params;
