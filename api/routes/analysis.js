...
export async function analyzeTaskComplexityHandler(req, res) {
	try {
		...
		const result = await analyzeTaskComplexityDirect(args, logger, {
			session: {}
		});
		...
		if (!result.success) {
			...
			return res.status(statusCode).json({
				...
				error: {
					code:
						statusCode === 404 ? 'TASK_NOT_FOUND' : 'ANALYZE_COMPLEXITY_ERROR',
					message: result.error
				}
			});
		}
		...
		res.json({
			...
			data: {
				...
				complexity:
					result.complexity ||
					result.data?.fullReport?.complexityAnalysis?.[0]?.complexityScore,
				factors:
					result.factors ||
					result.data?.fullReport?.complexityAnalysis?.[0]?.factors,
				recommendations:
					result.recommendations || result.data?.fullReport?.recommendations,
