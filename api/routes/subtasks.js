...
export async function updateSubtaskHandler(req, res) {
  try {
    ...
    if (!result.success) {
      ...
      if (result.error?.message?.includes('Task not found') || result.error?.code === 'TASK_NOT_FOUND') {
        statusCode = 404;
        errorCode = 'TASK_NOT_FOUND';
      } else if (result.error?.message?.includes('Subtask not found') || result.error?.code === 'SUBTASK_NOT_FOUND') {
        statusCode = 404;
