...
router.post(
	...
	async (req, res) => {
		try {
			...
			const lockStatus = await securityService.isAccountLocked(
				normalizedEmail,
				'email'
			);
				...
				if (
					...
					return res.status(403).json({
						...
						error: {
							...
							description:
								'アカウントにログインするには、メールアドレスの確認が必要です。受信トレイを確認して、確認リンクをクリックしてください。',
							actions: {
								resendVerification: {
...
router.post('/resend-verification', rateLimiters.auth, async (req, res) => {
	try {
		...
		// Validate input
		const errors = validateResendVerificationInput({ email });
		if (errors.length > 0) {
			return res.status(400).json({
				success: false,
				error: {
		...
		const { error } = await resendEmailVerification(normalizedEmail);
		...
		if (error) {
			...
			return res.status(500).json({
				...
				error: {
					code: 'INTERNAL_ERROR',
					message: '確認メールの送信中にエラーが発生しました'
				}
			});
		}

		// Log successful resend
		await logAuthEvent(AUDIT_EVENTS.AUTH_EMAIL_VERIFICATION_RESENT, {
			description: `Email verification resent for: ${normalizedEmail}`,
			ipAddress: req.ip,
			userAgent: req.get('User-Agent'),
			metadata: {
				email: normalizedEmail,
		...
		res.status(200).json({
			success: true,
			data: {
				message: '確認メールを再送信しました。受信トレイをご確認ください。',
				email: normalizedEmail
			}
		});
	} catch (error) {
