...
const taskCreateSchema = z
	...
	.refine((data) => data.prompt || (data.title && data.description), {
		...
		if (!result.success) {
			return res.status(400).json({
				...
				error: {
					...
					message:
						result.error?.message || result.error || 'Failed to create task'
				}
			});
		}
	...
	try {
		...
		if (data.prompt) {
			...
			if (data.testStrategy !== undefined)
				updates.testStrategy = data.testStrategy;
