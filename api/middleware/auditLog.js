import { auditLogger, AUDIT_EVENTS, RISK_LEVELS, DATA_SENSITIVITY } from '../services/auditLog.js';
...
const SENSITIVE_ENDPOINTS = {
  ...
  '/api/v1/auth/login': { eventType: AUDIT_EVENTS.AUTH_LOGIN_SUCCESS, riskLevel: RISK_LEVELS.MEDIUM },
  '/api/v1/auth/logout': { eventType: AUDIT_EVENTS.AUTH_LOGOUT, riskLevel: RISK_LEVELS.LOW },
  '/api/v1/auth/signup': { eventType: AUDIT_EVENTS.AUTH_SIGNUP, riskLevel: RISK_LEVELS.MEDIUM },
  '/api/v1/auth/forgot-password': { eventType: AUDIT_EVENTS.AUTH_PASSWORD_RESET_REQUEST, riskLevel: RISK_LEVELS.MEDIUM },
  '/api/v1/auth/reset-password': { eventType: AUDIT_EVENTS.AUTH_PASSWORD_RESET_SUCCESS, riskLevel: RISK_LEVELS.HIGH },
  '/api/v1/auth/change-password': { eventType: AUDIT_EVENTS.AUTH_PASSWORD_CHANGE, riskLevel: RISK_LEVELS.HIGH },

  // Organization endpoints
  '/api/v1/organizations': { eventType: AUDIT_EVENTS.ORG_CREATE, riskLevel: RISK_LEVELS.MEDIUM },
  '/api/v1/organizations/:id': { eventType: AUDIT_EVENTS.ORG_UPDATE, riskLevel: RISK_LEVELS.MEDIUM },
  '/api/v1/organizations/:id/members': { eventType: AUDIT_EVENTS.ORG_MEMBER_ADD, riskLevel: RISK_LEVELS.MEDIUM },
  '/api/v1/organizations/:id/invitations': { eventType: AUDIT_EVENTS.ORG_INVITATION_SEND, riskLevel: RISK_LEVELS.MEDIUM },

  // Project endpoints
  '/api/v1/projects': { eventType: AUDIT_EVENTS.PROJECT_CREATE, riskLevel: RISK_LEVELS.LOW },
  '/api/v1/projects/:id': { eventType: AUDIT_EVENTS.PROJECT_UPDATE, riskLevel: RISK_LEVELS.LOW },

  // Task endpoints
  '/api/v1/tasks': { eventType: AUDIT_EVENTS.TASK_CREATE, riskLevel: RISK_LEVELS.LOW },
  '/api/v1/tasks/:id': { eventType: AUDIT_EVENTS.TASK_UPDATE, riskLevel: RISK_LEVELS.LOW },
  '/api/v1/tasks/:id/status': { eventType: AUDIT_EVENTS.TASK_STATUS_CHANGE, riskLevel: RISK_LEVELS.LOW }
};
...
function determineEventConfig(req, res) {
  ...
  if (method === 'DELETE') {
    if (path.includes('/organizations/')) {
      return { eventType: AUDIT_EVENTS.ORG_DELETE, riskLevel: RISK_LEVELS.HIGH };
    }
    if (path.includes('/projects/')) {
      return { eventType: AUDIT_EVENTS.PROJECT_DELETE, riskLevel: RISK_LEVELS.MEDIUM };
    }
    if (path.includes('/tasks/')) {
      return { eventType: AUDIT_EVENTS.TASK_DELETE, riskLevel: RISK_LEVELS.LOW };
    }
  }
...
function isBulkOperation(req) {
  return BULK_OPERATION_PATTERNS.some(pattern => pattern.test(req.path)) ||
...
function isDataExport(req) {
  return DATA_EXPORT_PATTERNS.some(pattern => pattern.test(req.path)) ||
         req.query?.export === 'true' ||
         req.query?.format === 'csv';
}

function isSensitiveOperation(req) {
  return req.path.includes('/password') ||
...
export const auditErrorMiddleware = (err, req, res, next) => {
  if (err) {
    ...
    auditLogger.logEvent({
      eventType: isSecurityError ? AUDIT_EVENTS.SECURITY_UNAUTHORIZED_ACCESS : 'api.error',
      ...
      dataSensitivity: isSecurityError ? DATA_SENSITIVITY.CONFIDENTIAL : DATA_SENSITIVITY.INTERNAL,
      metadata: {
