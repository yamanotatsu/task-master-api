...
export function validatePassword(password, options = {}) {
	...
	if (password.length < defaultOptions.minLength) {
		issues.push(
			`Password must be at least ${defaultOptions.minLength} characters`
		);
	} else if (password.length > defaultOptions.maxLength) {
		issues.push(
			`Password must not exceed ${defaultOptions.maxLength} characters`
		);
	} else {
	...
	if (lowercase < defaultOptions.minLowercase) {
		issues.push(
			`Password must contain at least ${defaultOptions.minLowercase} lowercase letter(s)`
		);
	} else {
		score += 1;
	}
	...
	if (uppercase < defaultOptions.minUppercase) {
		issues.push(
			`Password must contain at least ${defaultOptions.minUppercase} uppercase letter(s)`
		);
	} else {
		score += 1;
	}
	...
	if (numbers < defaultOptions.minNumbers) {
		issues.push(
			`Password must contain at least ${defaultOptions.minNumbers} number(s)`
		);
	} else {
		score += 1;
	}
	...
	if (symbols < defaultOptions.minSymbols) {
		issues.push(
			`Password must contain at least ${defaultOptions.minSymbols} symbol(s)`
		);
	} else if (symbols > 0) {
		score += 2;
	}
...
export function isValidFullName(fullName, options = {}) {
	...
	if (
		trimmed.length < defaultOptions.minLength ||
		trimmed.length > defaultOptions.maxLength
	) {
		return false;
	}
...
export function isValidUrl(url, options = {}) {
	...
	try {
		...
		if (
			...
			!defaultOptions.protocols.includes(parsed.protocol.slice(0, -1))
		)
			return false;
...
export function validateFileUpload(file, options = {}) {
	...
	if (file.size > defaultOptions.maxSize) {
		errors.push(
			`File size exceeds maximum allowed size of ${defaultOptions.maxSize / 1024 / 1024}MB`
		);
	}

	// Type check
	if (
		defaultOptions.allowedTypes.length > 0 &&
	...
	switch (type) {
		...
		case 'enum':
			...
			if (options.values && Array.isArray(options.values)) {
				...
				if (!valid)
					errors.push(`Value must be one of: ${options.values.join(', ')}`);
			} else {
				valid = false;
...
/**
 * Validate email resend verification input
 * @param {Object} data - Input data
 * @returns {Array} Array of validation errors
 */
export function validateResendVerificationInput({ email }) {
	const errors = [];
