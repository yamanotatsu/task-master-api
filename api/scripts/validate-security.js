...
if (config.cors.origin) {
  results.push({ check: 'CORS Origin Policy', status: 'pass', message: 'Configured' });
  console.log(chalk.green('  ✓ Origin policy configured'));
} else {
  results.push({ check: 'CORS Origin Policy', status: 'warn', message: 'Open to all origins' });
  console.log(chalk.yellow('  ⚠ Open to all origins'));
}

if (config.cors.credentials) {
  results.push({ check: 'CORS Credentials', status: 'pass', message: 'Enabled' });
  console.log(chalk.green('  ✓ Credentials support enabled'));
} else {
  results.push({ check: 'CORS Credentials', status: 'fail', message: 'Disabled' });
...
if (config.csp.defaultSrc && config.csp.defaultSrc.includes("'self'")) {
  results.push({ check: 'CSP Default Source', status: 'pass', message: 'Restricted to self' });
  console.log(chalk.green('  ✓ Default source restricted to self'));
} else {
  results.push({ check: 'CSP Default Source', status: 'fail', message: 'Not properly restricted' });
  console.log(chalk.red('  ✗ Default source not properly restricted'));
}

if (config.csp.objectSrc && config.csp.objectSrc.includes("'none'")) {
  results.push({ check: 'CSP Object Source', status: 'pass', message: 'Blocked' });
  console.log(chalk.green('  ✓ Object source blocked'));
} else {
  results.push({ check: 'CSP Object Source', status: 'warn', message: 'Not blocked' });
...
if (config.hsts.maxAge > 0) {
  results.push({ check: 'HSTS Enabled', status: 'pass', message: `Max age: ${config.hsts.maxAge}s` });
  console.log(chalk.green(`  ✓ HSTS enabled (max age: ${config.hsts.maxAge}s)`));
  
  if (config.hsts.includeSubDomains) {
    results.push({ check: 'HSTS Subdomains', status: 'pass', message: 'Included' });
    console.log(chalk.green('  ✓ Includes subdomains'));
  } else {
    results.push({ check: 'HSTS Subdomains', status: 'warn', message: 'Not included' });
    console.log(chalk.yellow('  ⚠ Subdomains not included'));
  }
} else {
  results.push({ check: 'HSTS Enabled', status: config.isDevelopment ? 'info' : 'fail', message: 'Disabled' });
  const message = config.isDevelopment ? '  ℹ HSTS disabled (development)' : '  ✗ HSTS disabled in production';
...
requiredHeaders.forEach(header => {
  if (config.headers[header]) {
    results.push({ check: `Header: ${header}`, status: 'pass', message: config.headers[header] });
    console.log(chalk.green(`  ✓ ${header}: ${config.headers[header]}`));
  } else {
    results.push({ check: `Header: ${header}`, status: 'fail', message: 'Missing' });
...
if (rateLimitEnabled) {
  ...
  console.log(chalk.gray(`    Global: ${config.rateLimit.global.max} requests per ${config.rateLimit.global.windowMs}ms`));
  console.log(chalk.gray(`    Auth: ${config.rateLimit.auth.max} requests per ${config.rateLimit.auth.windowMs}ms`));
} else {
  results.push({ check: 'Rate Limiting', status: config.isDevelopment ? 'info' : 'warn', message: 'Disabled' });
  const message = config.isDevelopment ? '  ℹ Rate limiting disabled (development)' : '  ⚠ Rate limiting disabled';
  console.log(config.isDevelopment ? chalk.blue(message) : chalk.yellow(message));
}
...
restrictedFeatures.forEach(feature => {
  if (config.featurePolicy[feature] && config.featurePolicy[feature].includes("'none'")) {
...
if (featurePolicyConfigured) {
  results.push({ check: 'Permissions Policy', status: 'pass', message: 'Configured' });
} else {
  results.push({ check: 'Permissions Policy', status: 'warn', message: 'Not configured' });
...
if (config.isDevelopment) {
  ...
  console.log(chalk.gray('    • Security settings are relaxed for development'));
...
if (config.isProduction) {
  ...
  if (!process.env.SESSION_SECRET || process.env.SESSION_SECRET.length < 32) {
    results.push({ check: 'Session Secret', status: 'fail', message: 'Weak or missing' });
    console.log(chalk.red('    ✗ SESSION_SECRET should be at least 32 characters'));
  }
  
  if (!process.env.ALLOWED_ORIGINS) {
    results.push({ check: 'CORS Origins', status: 'warn', message: 'Not explicitly configured' });
    console.log(chalk.yellow('    ⚠ ALLOWED_ORIGINS not configured'));
  }
  
  if (!rateLimitEnabled) {
    console.log(chalk.red('    ✗ Rate limiting should be enabled in production'));
  }
}
