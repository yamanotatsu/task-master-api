...
import { auditMiddleware, auditRateLimitMiddleware, auditErrorMiddleware } from './middleware/auditLog.js';
...
if (process.env.ENABLE_RATE_LIMIT === 'true') {
  ...
  const strictLimiter = rateLimit(securityConfig.rateLimit.strict || securityConfig.rateLimit.auth);
...
app.use((req, res, next) => {
  ...
  console.log(`${new Date().toISOString()} - [${req.id || 'no-id'}] ${req.method} ${req.path} - ${req.ip}`);
  next();
});

// Apply audit logging middleware
app.use(auditMiddleware({ logAllRequests: process.env.AUDIT_LOG_ALL_REQUESTS === 'true' }));
...
app.get('/health', (req, res) => {
  res.json({ 
    ...
    security: {
      cors: 'enabled',
      csp: 'enabled',
      rateLimit: process.env.ENABLE_RATE_LIMIT === 'true' ? 'enabled' : 'disabled',
      headers: 'enhanced'
    }
  });
});
...
app.listen(PORT, () => {
  ...
  console.log(`  • CORS: ${securityConfig.cors.origin ? 'Restricted' : 'Open'}`);
  console.log(`  • CSP: Enabled with nonce support`);
  console.log(`  • HSTS: ${securityConfig.hsts.maxAge > 0 ? 'Enabled' : 'Disabled'}`);
  console.log(`  • Rate Limiting: ${process.env.ENABLE_RATE_LIMIT === 'true' ? 'Enabled' : 'Disabled'}`);
  console.log(`  • Trust Proxy: ${app.get('trust proxy') || 'Disabled'}`);
  console.log(`  • Security Headers: Enhanced`);
  console.log('═══════════════════════════════════════════════════════════════');
  
  if (!securityConfig.isProduction) {
    console.log(`Security status available at: http://localhost:${PORT}/api/v1/security/status`);
  }
});