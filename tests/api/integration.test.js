...
const createIntegrationTestApp = () => {
  ...
  const authMiddleware = async (req, res, next) => {
    ...
    try {
      ...
      const { data: { user }, error } = await supabaseTest.auth.getUser(token);
  ...
  orgRouter.post('/:organizationId/invites', authMiddleware, async (req, res) => {
...
describe('Integration Tests - End-to-End Authentication Flows', () => {
  ...
  describe('Complete User Registration Flow', () => {
    it('should complete full user registration and setup workflow', async () => {
      ...
      expect(updateResponse.body.data.profile.full_name).toBe(updateData.fullName);
  ...
  describe('Multi-User Organization Workflow', () => {
    it('should handle complete multi-user organization management flow', async () => {
      ...
      expectErrorResponse(unauthorizedInviteResponse, 'AUTHZ_REQUIRES_ADMIN', 403);
      ...
      expect(finalOrgsResponse.body.data.organizations[0].name).toBe(orgData.name);
    });
  });
  ...
  describe('Concurrent User Operations', () => {
    ...
    it('should handle concurrent organization creation', async () => {
      ...
      const orgPromises = Array(3).fill().map((_, i) =>
        request(app)
          .post('/api/v1/organizations')
      ...
      responses.forEach((response, i) => {
        ...
        expect(response.body.data.organization.name).toBe(`Concurrent Org ${i + 1}`);
      });
  ...
  describe('Cross-Organization Security', () => {
    it('should prevent cross-organization access violations', async () => {
      ...
      expectErrorResponse(unauthorizedAccessResponse, 'AUTHZ_REQUIRES_ADMIN', 403);
  ...
  describe('Data Consistency', () => {
    it('should maintain data consistency across operations', async () => {
      ...
      expect(profileResponse.body.data.profile.full_name).toBe('Updated Consistency Test User');
    });
  });
