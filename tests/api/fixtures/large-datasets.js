  ...
  for (let layer = 0; layer < layers; layer++) {
    ...
    for (let i = 0; i < tasksInLayer; i++) {
      ...
      if (layer > 0) {
        ...
        const depCount = Math.min(Math.floor(Math.random() * 3) + 1, taskCounter - 1);
        const startRange = Math.max(1, taskCounter - 20); // Look at last 20 tasks
        
        for (let j = 0; j < depCount; j++) {
          const depId = Math.floor(Math.random() * (taskCounter - startRange)) + startRange;
          const depTaskId = `task_${String(depId).padStart(3, '0')}`;
...
export const generateConcurrentOperations = (operationCount = 100) => {
  const operations = [];
  const taskIds = Array.from({ length: 20 }, (_, i) => `task_${String(i + 1).padStart(3, '0')}`);
  const operationTypes = ['create', 'update', 'delete', 'status_change', 'add_dependency', 'remove_dependency'];
...
function generateSectionTitle(index) {
  ...
  return titles[index % titles.length] + ` Module ${Math.floor(index / titles.length) + 1}`;
}
