...
export const malformedData = {
  ...
  deeplyNested: JSON.parse('{' + '"a":{'.repeat(100) + '"value":1' + '}'.repeat(100) + '}'),
  mixedTypes: {
    id: ['should', 'be', 'string'],
...
export const edgeCaseTasks = {
  ...
  maximalTask: {
    ...
    status: 'completed',
    priority: 'critical',
    dependencies: Array.from({ length: 100 }, (_, i) => `task_${String(i).padStart(3, '0')}`),
...
export const edgeCaseEnums = {
  ...
  invalidStatuses: ['', 'PENDING', 'Pending', 'done', 'cancelled', 'invalid', null, 123, true],
  validPriorities: ['low', 'medium', 'high', 'critical'],
  invalidPriorities: ['', 'LOW', 'Low', 'urgent', 'none', 'highest', null, 0, false]
};
...
export const edgeCaseArrays = {
  empty: [],
  single: ['task_001'],
  large: Array.from({ length: 1000 }, (_, i) => `task_${String(i).padStart(3, '0')}`),
