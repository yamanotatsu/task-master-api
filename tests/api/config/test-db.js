...
export const cleanupTestData = async () => {
  try {
    ...
    await supabaseAdmin.from('organization_members').delete().neq('id', '00000000-0000-0000-0000-000000000000');
    ...
    if (process.env.NODE_ENV === 'test') {
      await supabaseAdmin.auth.admin.listUsers().then(({ data: { users } }) => {
        return Promise.all(
          users
            .filter(user => user.email?.includes('test') || user.email?.includes('example'))
