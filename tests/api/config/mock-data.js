	...
	const defaultMocks = {
		auth: {
			...
			signInWithPassword: jest
				.fn()
				.mockResolvedValue(mockAuthResponses.loginSuccess),
			signOut: jest.fn().mockResolvedValue({ error: null }),
			getUser: jest.fn().mockResolvedValue(mockAuthResponses.getUserSuccess),
			refreshSession: jest
				.fn()
				.mockResolvedValue(mockAuthResponses.refreshSuccess),
			resetPasswordForEmail: jest.fn().mockResolvedValue({ error: null }),
			updateUser: jest.fn().mockResolvedValue(mockAuthResponses.getUserSuccess),
			admin: {
				createUser: jest
					.fn()
					.mockResolvedValue(mockAuthResponses.signupSuccess),
				deleteUser: jest.fn().mockResolvedValue({ error: null }),
				getUserByEmail: jest
					.fn()
					.mockResolvedValue(mockAuthResponses.getUserSuccess),
				listUsers: jest
					.fn()
		...
		from: jest.fn(() => ({
			...
			maybeSingle: jest
				.fn()
				.mockResolvedValue(mockDatabaseResponses.profileSelect),
	...
	const applyOverrides = (target, source) => {
		for (const key in source) {
			if (
				...
				!jest.isMockFunction(source[key])
			) {
