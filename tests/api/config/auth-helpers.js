...
export const expectErrorResponse = (response, expectedCode, expectedStatus = 400) => {
  expect(response.status).toBe(expectedStatus);
  expect(response.body).toMatchObject({
    success: false,
...
export const cleanupTestUser = async (supabaseAdmin, userId) => {
  try {
    await supabaseAdmin.from('organization_members').delete().eq('profile_id', userId);
...
export const cleanupTestOrganization = async (supabaseAdmin, organizationId) => {
  try {
    ...
    console.warn(`Failed to cleanup test organization ${organizationId}:`, error.message);
  }
};