	...
	describe('POST /api/v1/organizations', () => {
		describe('Success Cases', () => {
			it('should create organization with valid data', async () => {
				...
				expect(response.body.data.organization.description).toBe(
					orgData.description
				);
	...
	describe('GET /api/v1/organizations/:organizationId/members', () => {
		describe('Success Cases', () => {
			...
			it('should filter members by role', async () => {
				const response = await request(app)
					.get(
						`/api/v1/organizations/${testOrganization.id}/members?role=admin`
					)
					.set('Authorization', `Bearer ${adminTokens.accessToken}`)
					.expect(200);
			...
			it('should search members by name or email', async () => {
				const response = await request(app)
					.get(
						`/api/v1/organizations/${testOrganization.id}/members?search=admin`
					)
					.set('Authorization', `Bearer ${adminTokens.accessToken}`)
					.expect(200);
			...
			it('should handle pagination', async () => {
				const response = await request(app)
					.get(
						`/api/v1/organizations/${testOrganization.id}/members?page=1&limit=5`
					)
					.set('Authorization', `Bearer ${adminTokens.accessToken}`)
					.expect(200);
	...
	describe('PUT /api/v1/organizations/:organizationId/members/:profileId', () => {
		describe('Success Cases', () => {
			it('should update member role as admin', async () => {
				const response = await request(app)
					.put(
						`/api/v1/organizations/${testOrganization.id}/members/${memberUser.id}`
					)
					.set('Authorization', `Bearer ${adminTokens.accessToken}`)
					.send({ role: 'admin' })
					.expect(200);
		...
		describe('Authorization Errors', () => {
			it('should reject role update by non-admin members', async () => {
				const response = await request(app)
					.put(
						`/api/v1/organizations/${testOrganization.id}/members/${adminUser.id}`
					)
					.set('Authorization', `Bearer ${memberTokens.accessToken}`)
					.send({ role: 'member' })
					.expect(403);
		...
		describe('Not Found Errors', () => {
			it('should return 404 for non-existent member', async () => {
				const response = await request(app)
					.put(
						`/api/v1/organizations/${testOrganization.id}/members/non-existent-id`
					)
					.set('Authorization', `Bearer ${adminTokens.accessToken}`)
					.send({ role: 'admin' })
					.expect(404);
