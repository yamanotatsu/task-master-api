...
function createIntegrationTestApp() {
	...
	app.get('/api/v1/tasks', async (req, res) => {
		const result = await mockDependencies.listTasksDirect(
			{},
	...
	app.get('/api/v1/tasks/:id', async (req, res) => {
		...
		const result = await mockDependencies.showTaskDirect(
			{ taskId },
			mockDependencies.logger,
			{}
		);
	...
	app.post('/api/v1/tasks', async (req, res) => {
		...
		const result = await mockDependencies.addTaskDirect(
			req.body,
			mockDependencies.logger,
			{}
		);
	...
	app.put('/api/v1/tasks/:id', async (req, res) => {
		...
		const result = await mockDependencies.updateTaskByIdDirect(
			{ taskId, updates: req.body },
			mockDependencies.logger,
			{}
		);
		if (!result.success) {
			return res.status(404).json({
				success: false,
				error: { code: 'UPDATE_TASK_ERROR', message: result.error }
			});
		}
		res.json({
			success: true,
	...
	app.delete('/api/v1/tasks/:id', async (req, res) => {
		...
		const result = await mockDependencies.removeTaskDirect(
			{ taskId },
			mockDependencies.logger,
			{}
		);
		if (!result.success) {
			return res.status(404).json({
				success: false,
				error: { code: 'DELETE_TASK_ERROR', message: result.error }
			});
		}
	...
	app.patch('/api/v1/tasks/:id/status', async (req, res) => {
		...
		const result = await mockDependencies.setTaskStatusDirect(
			{ taskId, status: req.body.status },
			mockDependencies.logger,
			{}
		);
		if (!result.success) {
			return res.status(404).json({
				success: false,
				error: { code: 'UPDATE_STATUS_ERROR', message: result.error }
			});
		}
		res.json({
			success: true,
			data: result.data,
			message: `Task ${taskId} status updated to ${req.body.status}`
		});
	});
	...
	app.post('/api/v1/tasks/:id/subtasks', async (req, res) => {
		...
		if (!req.body.title) {
			return res.status(400).json({
				success: false,
				error: { code: 'INVALID_INPUT', message: 'Subtask title is required' }
			});
		}

		const result = await mockDependencies.addSubtaskDirect(
			{ parentTaskId, ...req.body },
			mockDependencies.logger,
			{}
		);
		if (!result.success) {
			return res.status(404).json({
				success: false,
				error: { code: 'ADD_SUBTASK_ERROR', message: result.error }
			});
		}
	...
	app.post('/api/v1/tasks/:id/dependencies', async (req, res) => {
		...
		if (isNaN(dependencyId)) {
			return res.status(400).json({
				...
				error: {
					code: 'INVALID_INPUT',
					message: 'Dependency ID must be a number'
				}
			});
		}

		const result = await mockDependencies.addDependencyDirect(
			{ taskId, dependencyId },
			mockDependencies.logger,
			{}
		);
		if (!result.success) {
			return res.status(400).json({
				success: false,
				error: { code: 'ADD_DEPENDENCY_ERROR', message: result.error }
			});
		}
	...
	app.post('/api/v1/tasks/:id/expand', async (req, res) => {
		...
		const result = await mockDependencies.expandTaskDirect(
			{ id: taskId, ...req.body },
			mockDependencies.logger,
			{}
		);
		if (!result.success) {
			return res.status(400).json({
				success: false,
				error: { code: 'EXPAND_TASK_ERROR', message: result.error }
			});
		}
		res.json({ success: true, data: result });
	});
	...
	app.post('/api/v1/tasks/generate-from-prd', async (req, res) => {
		if (!req.body.prd_content) {
			return res.status(400).json({
				success: false,
				error: { code: 'INVALID_INPUT', message: 'PRD content is required' }
			});
		}

		const result = await mockDependencies.generateTasksFromPrdDirect(
			{ prdContent: req.body.prd_content },
			mockDependencies.logger,
			{}
		);
		if (!result.success) {
			return res.status(500).json({
				success: false,
				error: { code: 'TASK_GENERATION_ERROR', message: result.error }
			});
		}
		res.json({ success: true, data: result });
	});
		...
		test('should handle task status progression workflow', async () => {
			...
			for (let i = 0; i < taskStatuses.length; i++) {
				...
				expect(response.body.message).toBe(
					`Task 1 status updated to ${status}`
				);
			}
