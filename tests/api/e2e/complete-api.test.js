...
} from '../fixtures/large-datasets.js';
...
function createE2EApp() {
	...
	const findTask = (id) => {
		const numId =
			typeof id === 'string' ? parseInt(id.replace('task_', '')) : id;
	...
	app.get('/api/v1/project', (req, res) => {
		res.json({
			...
			data: {
				...
				completedTasks: Array.from(taskStore.values()).filter(
	...
	app.delete('/api/v1/tasks/:id', (req, res) => {
		...
		const dependentTasks = Array.from(taskStore.values()).filter(
			(t) => t.dependencies && t.dependencies.includes(taskKey)
		);
...
describe('E2E API Tests - Complete Functionality', () => {
	...
	describe('Complete Task Management Workflow', () => {
		test('should handle full task lifecycle', async () => {
			...
			const initialList = await request(app).get('/api/v1/tasks').expect(200);
			...
			const updatedList = await request(app).get('/api/v1/tasks').expect(200);
			...
			await request(app).get(`/api/v1/tasks/${taskId}`).expect(404);
		});
	...
	describe('Error Handling and Edge Cases', () => {
		test('should handle validation errors', async () => {
			...
			// Invalid task ID
			await request(app).get('/api/v1/tasks/invalid-id').expect(400);
		});
		...
		test('should handle deletion constraints', async () => {
			...
			await request(app).delete(`/api/v1/tasks/${task1Id}`).expect(400);
