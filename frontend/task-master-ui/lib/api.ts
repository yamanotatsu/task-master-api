...
class ApiClient {
      ...
      if (!error) {
        ...
        if (newToken) {
          ...
          if (!retryResponse.ok || (retryData.success === false)) {
            throw new Error(retryData.error?.message || `API Error: ${retryResponse.statusText}`);
          }
  ...
  async deleteAccount(password: string, confirmDeletion: string): Promise<void> {
  ...
  async getUserProfile(): Promise<Member> {
    const response = await this.fetchAPI<{ profile: Member }>('/api/v1/users/profile');
  ...
  async getOrganization(id: string): Promise<{
    organization: Organization;
    membership: { role: string; joinedAt: string };
    statistics: { memberCount: number; projectCount: number; activeTaskCount: number };
  }> {
  ...
  async inviteMember(organizationId: string, email: string, role: 'admin' | 'member' = 'member'): Promise<{
  ...
  async getOrganizationMembers(
    ...
    return this.fetchAPI(`/api/v1/organizations/${organizationId}/members${queryString}`);
  }

  async updateMemberRole(organizationId: string, profileId: string, role: 'admin' | 'member'): Promise<Member> {
  ...
  async removeOrganizationMember(organizationId: string, profileId: string): Promise<void> {
    await this.fetchAPI(`/api/v1/organizations/${organizationId}/members/${profileId}`, {
      method: 'DELETE',
    });
  }
  ...
  async acceptInvitation(token: string): Promise<{
    ...
    return this.fetchAPI(`/api/v1/invitations/${token}/accept`, {
      method: 'POST',
    });
  }
  ...
  async getOrganizationWrapped(organizationId: string): Promise<ApiResponse<Organization>> {
  ...
  async getOrganizationMembersWrapped(organizationId: string): Promise<ApiResponse<Member[]>> {
    const response = await this.getOrganizationMembers(organizationId);
    return { success: true, data: response.members };
  }

  async inviteMemberWrapped(organizationId: string, data: { email: string; role: 'admin' | 'member' }): Promise<ApiResponse<Invitation>> {
  ...
  async getOrganizationInvitations(organizationId: string): Promise<ApiResponse<Invitation[]>> {
  ...
  async cancelInvitation(organizationId: string, invitationId: string): Promise<void> {
    await this.fetchAPI(`/api/v1/organizations/${organizationId}/invitations/${invitationId}`, {
      method: 'DELETE',
    });
  }

  async resendInvitation(organizationId: string, invitationId: string): Promise<void> {
    await this.fetchAPI(`/api/v1/organizations/${organizationId}/invitations/${invitationId}/resend`, {
      method: 'POST',
    });
  }

  async updateOrganizationMember(organizationId: string, memberId: string, data: { role: 'admin' | 'member' }): Promise<ApiResponse<Member>> {
    const response = await this.updateMemberRole(organizationId, memberId, data.role);
    return { success: true, data: response };
  }

  // Token refresh
  async refreshToken(refreshToken: string): Promise<AuthTokens> {
    const response = await this.fetchAPI<{ tokens: AuthTokens }>('/api/v1/auth/refresh', {
      method: 'POST',
