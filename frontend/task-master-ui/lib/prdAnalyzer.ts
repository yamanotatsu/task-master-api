...
export class PRDAnalyzer {
	...
	static generateNextQuestion(analysis: PRDAnalysisResult): string | null {
		...
		const questions = {
			overview:
				'このプロジェクトの主な目的と、解決したい具体的な問題について教えてください。また、想定されるターゲットユーザーは誰ですか？',
			features:
				'このシステムの主要な機能を3-5個挙げていただけますか？それぞれの機能について、ユーザーがどのように使うかも含めて説明してください。',
			technical:
				'技術的な実装について、使用予定のプログラミング言語、フレームワーク、データベースなどを教えてください。',
			metrics:
				'このプロジェクトの成功をどのように測定しますか？具体的なKPIや目標値があれば教えてください。',
			timeline:
				'プロジェクトの予定スケジュールを教えてください。主要なマイルストーンやリリース予定日はありますか？'
		};
