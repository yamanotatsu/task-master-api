...
describe('API Client', () => {
	...
	describe('Members', () => {
		...
		it('updates member role successfully', async () => {
			...
			const result = await apiClient.members.updateRole(
				'org-123',
				'member-123',
				'admin'
			);
	...
	describe('Error Handling', () => {
		it('handles database errors', async () => {
			...
			await expect(apiClient.organizations.list('user-123')).rejects.toThrow(
		...
		it('handles network errors', async () => {
			mockSupabaseClient
				.from()
				.select()
				.eq.mockRejectedValue(new Error('Network error'));
	...
	describe('Query Building', () => {
		it('builds complex queries with filters', async () => {
			...
			expect(mockSupabaseClient.from().select().eq().neq).toHaveBeenCalledWith(
				'status',
				'inactive'
			);
		...
		it('supports pagination', async () => {
			...
			expect(mockSupabaseClient.from().select().range).toHaveBeenCalledWith(
				20,
				29
			);
			expect(result).toEqual(mockData);
		...
		it('supports text search', async () => {
			...
			expect(mockSupabaseClient.from().select().ilike).toHaveBeenCalledWith(
				'name',
				'%test%'
			);
			expect(result).toEqual(mockData);
	...
	describe('Real-time Subscriptions', () => {
		it('subscribes to organization changes', () => {
			...
			const subscription = apiClient.organizations.subscribe(
				'org-123',
				callback
			);
