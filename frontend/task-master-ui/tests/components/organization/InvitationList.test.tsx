...
describe('InvitationList', () => {
	...
	it('confirms before canceling invitation', async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText(/are you sure you want to cancel this invitation/i)
			).toBeInTheDocument();
			expect(
	...
	it('shows limited options for expired invitations', async () => {
		...
		const expiredInvitation = screen
			.getByText('expired@example.com')
			.closest('[data-testid="invitation-row"]');
		const actionMenu = expiredInvitation.querySelector(
			'[role="button"][aria-label*="invitation options"]'
		);
	...
	it('filters invitations by role', async () => {
		...
		await waitFor(() => {
			expect(screen.getByText('pending2@example.com')).toBeInTheDocument();
			expect(
				screen.queryByText('pending1@example.com')
	...
	it('searches invitations by email', async () => {
		...
		await waitFor(() => {
			expect(screen.getByText('pending1@example.com')).toBeInTheDocument();
			expect(
				screen.queryByText('pending2@example.com')
			).not.toBeInTheDocument();
		});
	});
	...
	it('shows empty state when no invitations', () => {
		...
		expect(
			screen.getByText(/invite team members to get started/i)
		).toBeInTheDocument();
	});
	...
	it('shows bulk actions for multiple selections', async () => {
		...
		await waitFor(() => {
			...
			expect(
				screen.getByRole('button', { name: /resend selected/i })
			).toBeInTheDocument();
			expect(
				screen.getByRole('button', { name: /cancel selected/i })
			).toBeInTheDocument();
		});
	});
	...
	it('performs bulk resend operation', async () => {
		...
		const resendSelectedButton = screen.getByRole('button', {
			name: /resend selected/i
		});
		...
		await waitFor(() => {
			...
			expect(toast.success).toHaveBeenCalledWith(
				'2 invitations resent successfully'
			);
		});
	});
});
