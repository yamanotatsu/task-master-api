...
describe('OrganizationSetupForm', () => {
	...
	it('validates required fields', async () => {
		...
		await waitFor(() => {
			...
			expect(
				screen.getByText(/organization slug is required/i)
			).toBeInTheDocument();
		});
	...
	it('validates organization name length', async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText(/organization name must be less than 50 characters/i)
			).toBeInTheDocument();
		});
	...
	it('shows organization benefits section', () => {
		...
		expect(
			screen.getByText(/collaborate with team members/i)
		).toBeInTheDocument();
		expect(screen.getByText(/manage projects together/i)).toBeInTheDocument();
		expect(
			screen.getByText(/control access and permissions/i)
		).toBeInTheDocument();
	});
