...
const TaskSelectionContext = createContext<
	TaskSelectionContextValue | undefined
>(undefined);
...
export const useTaskSelectionContext = () => {
	...
	if (!context) {
		throw new Error(
			'useTaskSelectionContext must be used within TaskSelectionProvider'
		);
	}
	return context;
};
...
export const TaskSelectionProvider: React.FC<TaskSelectionProviderProps> = ({
	...
	const selectTasksInRange = useCallback(
		(startId: number, endId: number, allTaskIds: number[]) => {
