...
import { ChevronRight, ChevronDown, Plus, MoreHorizontal, ArrowUpRight } from 'lucide-react';
import { cn } from '@/lib/utils';
import { EditableCell, StatusSelect, PrioritySelect, PersonTag, DatePicker } from '@/components/ui/notion';
      ...
      >
        ...
        <td className="px-2 py-2">
          <div className="group/title relative flex items-center gap-2">
            <div className="flex-1 transition-transform hover:translate-y-[-1px] hover:shadow-sm rounded">
              <EditableCell
                value={task.title}
                onValueChange={(value) => onTaskUpdate(task.id, { title: value })}
                placeholder="タスク名を入力"
              />
            </div>
            <button
        ...
        <td className="px-2 py-2 w-32">
          <DatePicker
            ...
            onDateChange={(date) => onTaskUpdate(task.id, { deadline: date?.toISOString() })}
          />
        </td>
        ...
        <>
          {subtasks.map((subtask) => (
            <SubtaskRow
              ...
              taskId={task.id}
              projectId={projectId}
              onSubtaskClick={(subtaskId) => onSubtaskClick?.(task.id, subtaskId)}
            />
          ))}
