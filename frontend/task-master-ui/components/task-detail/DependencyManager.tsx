...
export const DependencyManager: React.FC<DependencyManagerProps> = ({
  ...
  const handleAddDependency = (selectedTaskId: number) => {
    ...
    if (selectedTask && wouldCreateCircularDependency(task, selectedTask, allTasks)) {
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        {blockingTasks.length === 0 ? (
          <p className="text-sm text-gray-500">このタスクに依存するタスクはありません</p>
        ) : (
          <div className="space-y-2">
            {blockingTasks.map(blockingTask => (
              ...
              >
                ...
                <span className="text-sm text-gray-600">{blockingTask.title}</span>
