...
export const useBatchOperations = (onTasksUpdate?: (tasks: Task[]) => void) => {
	...
	const batchUpdateTasks = useCallback(
		async (taskIds: number[], updates: Partial<Task>): Promise<Task[]> => {
	...
	const batchUpdateStatus = useCallback(
		async (taskIds: number[], status: Task['status']) => {
	...
	const batchUpdatePriority = useCallback(
		async (taskIds: number[], priority: Task['priority']) => {
			return batchUpdateTasks(taskIds, { priority });
		},
		[batchUpdateTasks]
	);

	const batchUpdateAssignee = useCallback(
		async (taskIds: number[], assignee: string) => {
