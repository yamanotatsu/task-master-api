...
export default function MembersPage() {
	...
	const loadOrganizationData = async () => {
		try {
			...
			setCurrentUserRole(currentOrganization.role || 'member');

			// Load members
			const { data: membersData } = await api.getOrganizationMembersWrapped(
			...
			if (currentOrganization.role === 'admin') {
				const { data: invitesData } = await api.getOrganizationInvitations(
					currentOrganization.id
				);
				if (invitesData) {
	...
	const handleInviteMember = async (
		...
		try {
			...
			await api.inviteMemberWrapped(currentOrganization.id, { email, role });
	...
	const handleUpdateMemberRole = async (
		...
		try {
			...
			await api.updateOrganizationMember(currentOrganization.id, memberId, {
				role: newRole
			});
	...
	const handleCancelInvitation = async (invitationId: string) => {
		try {
			...
			await api.cancelInvitation(currentOrganization.id, invitationId);
	...
	const handleResendInvitation = async (invitationId: string) => {
		try {
			...
			await api.resendInvitation(currentOrganization.id, invitationId);
		...
		<div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6 animate-fade-in">
			...
			{filteredMembers.length === 0 && filteredInvitations.length === 0 ? (
				<EmptyState
					...
					title={
						searchQuery ? 'メンバーが見つかりません' : 'メンバーがいません'
					}
					description={
						searchQuery
							? '検索条件を変更してください'
							: '新しいメンバーを招待してチームを構築しましょう'
					}
					action={
						!searchQuery && isAdmin
							? {
