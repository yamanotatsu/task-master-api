...
import { ArrowLeft, ArrowRight, Check, MessageSquare, FileText, Settings2 } from "lucide-react"
...
function NewProjectPage() {
  ...
  // Step 2 State
  const [conversationMode, setConversationMode] = useState<ConversationMode>("guided")
  ...
  const checkPRDQuality = (content: string) => {
    const quality: PRDQuality = {
      ...
      hasTechnicalRequirements: /技術|technical|アーキテクチャ|architecture/i.test(content),
  ...
  const handleNextStep = () => {
    if (currentStep === 1) {
      ...
      const initialMessage: Message = {
        ...
        role: "ai",
        content: conversationMode === "guided" 
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const { aiResponse, prdQualityScore, missingRequirements, sectionScores: scores } = await api.sendAIDialogue(
        sessionId,
        inputMessage,
        conversationMode
      )
                        ...
                        <div key={key} className="space-y-1">
                          ...
                          <div className="w-full bg-gray-200 rounded-full h-1.5">
                            <div
                              className={cn(
                                ...
                                sectionData.score >= 70 ? "bg-green-500" :
                                sectionData.score >= 40 ? "bg-yellow-500" :
                                "bg-red-500"
                              )}
                          ...
                          {sectionData.missing.length > 0 && (
                            <div className="text-xs text-gray-500 mt-1">
                              ...
                              {sectionData.missing.length > 2 && ` 他${sectionData.missing.length - 2}項目`}
                            </div>
                          )}
                        </div>
                      )
